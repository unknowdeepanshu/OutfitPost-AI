...
async function GeneratedTaskId(payload: payload) {
  try {
    const BagMerge = await instance.post<CreateTaskResponse>(
      "/enhance",
      payload,
      {
        headers: {
...
async function GeneratedMergeImage(taskId: string) {
  try {
    const imageId = await instance.get<GetTaskResponse>(`/enhance/${taskId}`);
    return imageId.data;
  } catch (error) {
    if (axios.isAxiosError(error)) {
      console.error("YouCam API Error:", {
...
export default async function AIPhotoEnhance(payload: payload) {
