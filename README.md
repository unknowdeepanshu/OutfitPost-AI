...
OutfitPost-AI uses a Clerk webhook to synchronize newly created Clerk users with the application's MongoDB database.

The webhook endpoint is:

```text
POST /api/v1/auth/createduserwebhook
```

### Local Development

Clerk needs to reach the backend through a publicly accessible HTTPS URL. For local development, you can use ngrok.
...
In your Clerk Dashboard, create a webhook endpoint using:

```text
https://your-subdomain.ngrok-free.app/api/v1/auth/createduserwebhook
```

Enable the required user event:

user.created and user.deleted

After creating the webhook, copy the **Signing Secret** generated by Clerk and add it to:

```env
CLERK_WEBHOOK_SIGNING_SECRET=your_webhook_signing_secret
```

Restart the gateway service after adding the environment variable.

> **Important:** The ngrok URL can change when the tunnel is restarted. If the URL changes, update the webhook endpoint in Clerk.

### What the Webhook Does

When a user creates an account through Clerk:
...
Gateway
  │
  ▼
/api/v1/auth/createduserwebhook
  │
  ▼
MongoDB
```

The webhook allows the backend to create/synchronize the corresponding user record in MongoDB.

Without configuring the Clerk webhook, Clerk authentication can still work, but the application's MongoDB user synchronization will not work as intended.
